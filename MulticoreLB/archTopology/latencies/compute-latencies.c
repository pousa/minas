...
coreFactor (int i, int j)
{
  ...
  if (i != j)
    {
	core1 = hwloc_get_pu_obj_by_os_index(topo,i);
	core2 = hwloc_get_pu_obj_by_os_index(topo,j);
      	ancestor = hwloc_get_common_ancestor_obj (topo, core1, core2);

//printf("Cores and ancestor: %d, %d, %d %d\n",i,j,ancestor->type, ancestor->depth);
    
      if (ancestor->type == HWLOC_OBJ_CACHE){
	res = cache_latencies[ancestor->attr->cache.depth - 1];
		//printf("Core %d - Core %d have shared cache at level %d\n", i,j,ancestor->attr->cache.depth);
	}
      else
	res = nodes_matrix[cpus_to_nodes[i]][cpus_to_nodes[j]];
    }
  else
...
//printf("cache latencies: %lf %lf %lf\n", cache_latencies[0], cache_latencies[1], cache_latencies[2]);
