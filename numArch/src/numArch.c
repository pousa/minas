...
float na_numafactor(int node1, int node2)
{
  ...
  fp = fopen("/tmp/output/numafactor.minas","r");
...
/*Compute Ids to place threads and returns a vector of Ids
 ...
 share=1 : share nodes
*/
int* na_computeIds_Nodes(int numThreads,int share,int helperT)
{
  int *core_ids,num_nodes,i,j,k,*tmp_cores,cont_itens;
  int max_nodes = na_get_maxnodes();
  int max_cores = na_get_cpusnode(); //max number of cores per node

  cont_itens = 0;
  core_ids = malloc(numThreads*sizeof(int));
  tmp_cores = malloc(max_cores*sizeof(int));

/*Helper Threads MUST be considered*/
if(helperT)
{
	if(share){/*Want to share node*/

     		num_nodes = (2*numThreads)/(max_cores);
     		num_nodes += ((numThreads%max_cores)>0)?1:0;

     		for(i=0;i<num_nodes;i++) // for each node get cores ids
     		{
			tmp_cores=na_get_cpusidnode(i);
			for(j=cont_itens,k=0;k<max_cores && j<numThreads;j++,k=k+2)
			{  core_ids[j]=tmp_cores[k]; cont_itens++;}		
     		}

   	}
        else
	{
		/*avoid node sharing if possible, if not reduce node sharing*/
   		if(numThreads < max_nodes)
    		{
			for(i=0;i<numThreads;i++)
			{
	    			tmp_cores=na_get_cpusidnode(i);
				for(j=cont_itens,k=0;k<1 && j<numThreads;j++,k++)//get the first core of the node
					{  core_ids[j]=tmp_cores[k]; cont_itens++;}		
			}
    		}   
    		else //more than one thread per node - reduce node sharing
     		{
			k=0;
			for(i=0;i<numThreads;i++)
			{
				if((i%max_nodes == 0)&& (i != 0)) k+=2;
	    			tmp_cores=na_get_cpusidnode(i%max_nodes);
				core_ids[i]=tmp_cores[k]; 		
			}
     		} 
	}
 ...
 /*Want to share node*/
   if(share){	
     num_nodes = numThreads/max_cores;
     num_nodes += ((numThreads%max_cores)>0)?1:0;

     for(i=0;i<num_nodes;i++) // for each node get cores ids
     {
	tmp_cores=na_get_cpusidnode(i);
	for(j=cont_itens,k=0;k<max_cores && j<numThreads;j++,k++)
	{  core_ids[j]=tmp_cores[k]; cont_itens++;}		
     }
 }
 else{/*avoid node sharing if possible, if not reduce node sharing*/
    if(numThreads <= max_nodes)
    {
	for(i=0;i<numThreads;i++)
	{
...
/*UMA - Compute Ids to place threads and returns a vector of Ids
 share=0 : avoid to share cache
 share=1 : share cache
*/
int* na_computeIds_Cores(int numThreads,int share,int helperT)
{
  int *core_ids,num_sockets,i,j,k,*tmp_cores,cont_itens;
  int max_cores = na_get_cpusnode(); //max number of cores
  int max_cores_socket = na_get_cpussockets(); //max number of cores per socket
  int max_sockets = na_get_sockets();
  int n_cores_share = na_get_cache_cores(); //number of cores that share a cache level
...
if(helperT)
{
 	/*Want to share caches*/
 	if(share){
		num_sockets = (2*numThreads)/max_cores_socket;
     		num_sockets += ((numThreads%max_cores_socket)>0)?1:0;

     		for(i=0;i<num_sockets;i++) // for each socket get cores ids
		{		tmp_cores=na_get_cpusidsocket(i);
			for(j=cont_itens,k=0;k<max_cores_socket && j<numThreads;j++,k+=2)
				{  core_ids[j]=tmp_cores[k]; cont_itens++;}		
     		}
   	}
        else
        {
	    if(numThreads <= max_sockets)
   		 {
			for(i=0;i<numThreads;i++)
			{
	    			tmp_cores=na_get_cpusidsocket(i);
				for(j=cont_itens,k=0;k<1 && j<numThreads;j++,k++)//get the first core of the socket
				{  core_ids[j]=tmp_cores[k]; cont_itens++;}		
			}
   		 }   
    		else //more than one thread per socket - reduce cache sharing
     		{
			k=0;
			for(i=0;i<numThreads;i++)
			{
				if(i%max_cores_socket == 0) k+=2;
	    			tmp_cores=na_get_cpusidsocket(i%max_cores_socket);
				core_ids[i]=tmp_cores[k]; 		
			}
     		} 
	}
 ...
 /*Want to share caches*/
 if(share){
   if(numThreads <= max_cores_socket)
     core_ids = na_get_cpusidsocket(0);//always use socket zero as default
   else
   {
     num_sockets = numThreads/max_cores_socket;
     num_sockets += ((numThreads%max_cores_socket)>0)?1:0;

     for(i=0;i<num_sockets;i++) // for each socket get cores ids
     {
	tmp_cores=na_get_cpusidsocket(i);
	for(j=cont_itens,k=0;k<max_cores_socket && j<numThreads;j++,k++)
	{  core_ids[j]=tmp_cores[k]; cont_itens++;}		
     }
   }
 }
 else{/*avoid cache sharing if possible, if not reduce cache sharing*/
    if(numThreads <= max_sockets)
    {
	for(i=0;i<numThreads;i++)
	{
	...
	for(i=0;i<numThreads;i++)
	{
		if(i%max_sockets == 0 && i != 0) k+=n_cores_share;
	    	tmp_cores=na_get_cpusidsocket(i%max_sockets);
		core_ids[i]=tmp_cores[k]; 		
	}
     } 
 }
...
/*Get cores considering max_share of memories*/
int* na_max_share_ht(int numThreads)
{
  int *cores=NULL;
  cores = malloc(numThreads*sizeof(int));

  int num_nodes,i,j,k,*tmp_cores,cont_itens;
  int max_nodes = na_get_maxnodes();
  int max_cores = na_get_cpusnode(); //max number of cores per node
  ...
  if(na_is_numa())
  {
   		...
   		num_nodes = (2*numThreads)/max_cores;
     		...
     		{
			tmp_cores=na_get_cpusidnode(i);
			for(j=cont_itens,k=1;k<max_cores && j<numThreads;j++,k+=2)
			{  cores[j]=tmp_cores[k]; cont_itens++;}		
     		}
  ...
  else
  {
    		...
    		num_sockets = (2*numThreads)/max_cores_socket;
     		...
     		{
			tmp_cores=na_get_cpusidsocket(i);
			for(j=cont_itens,k=1;k<max_cores_socket && j<numThreads;j++,k+=2)
				{  cores[j]=tmp_cores[k]; cont_itens++;}		
     		}
...
/*Get cores considering min_share of memories*/
int* na_min_share_ht(int numThreads)
{
  int *cores=NULL;
  cores = malloc(numThreads*sizeof(int));

  int num_nodes,i,j,k,*tmp_cores,cont_itens,core_i;
  int max_nodes = na_get_maxnodes();
  int max_cores = na_get_cpusnode(); //max number of cores per node
  ...
  if(na_is_numa())
  {
   		...
   		if(numThreads < max_nodes)
			...
			{
	    			tmp_cores=na_get_cpusidnode(i);
				for(j=cont_itens,k=1;k<2 && j<numThreads;j++,k=k+2)//get the first core of the node
					{  cores[j]=tmp_cores[k]; cont_itens++;}		
			}
    		}   
    		else //more than one thread per node - reduce node sharing
			...
			for(i=0;i<numThreads;i++)
			{
				if((i%max_nodes == 0) && (i != 0)) k+=2;
				core_i = i%max_nodes;
				tmp_cores=na_get_cpusidnode(core_i);
				cores[i]=tmp_cores[k]; 		
			}
     		} 
  }
  else
  {
 	if(numThreads <= max_sockets)
		...
		{
	    	tmp_cores=na_get_cpusidsocket(i);
		for(j=cont_itens,k=1;k<2 && j<numThreads;j++,k++)//get the first core of the socket
			{  cores[j]=tmp_cores[k]; cont_itens++;}		
		}
    	}   
    	else //more than one thread per socket - reduce cache sharing
...
/*
 *Create a vector of core ids to use in the workstealing
 * */
int* na_workStealing_max(int numThreads,int helperT)
{
 int *cores;
 cores = malloc(numThreads*(sizeof(int)));
 
 if(na_is_numa())
   cores =  na_computeIds_Nodes(numThreads,1,helperT);
 else
   cores = na_computeIds_Cores(numThreads,1,helperT); 
 ...
 *Create a vector of core ids to use in the workstealing
 * */
int* na_workStealing_min(int numThreads,int helperT)
{
 int *cores,*cores_max,*cores_min;
 int i,j,k,maxcpus;

 maxcpus = na_get_maxcpus();
 cores_min = malloc(numThreads*(sizeof(int)));
 cores_max = malloc(maxcpus*(sizeof(int)));
 cores = malloc(numThreads*(sizeof(int)));

 
 if(na_is_numa()){
   cores_max =  na_computeIds_Nodes(maxcpus,1,0);
   cores_min =  na_computeIds_Nodes(numThreads,0,helperT);
 }
 else{
   cores_max = na_computeIds_Cores(maxcpus,1,0); 
   cores_min =  na_computeIds_Cores(numThreads,0,helperT);
 }   

 k=0;
 for(i=0;i<maxcpus;i++) {
	 j=0;
	 while(j<numThreads && cores_max[i] != cores_min[j])j++;
    	 if(cores_max[i] == cores_min[j]){
      		cores[k] = cores_max[i];	  
      		k++;}
 }  
...
int* na_cores_Placement(int numThreads,int share,int helperT)
{
  FILE *file;
  int *cores,tmp,i;

  if(share)
    file = fopen("/tmp/output/intra_placement.minas","r");
   else
    file = fopen("/tmp/output/inter_placement.minas","r");
  
  if(file==NULL) return NULL;

  if(helperT){
	    cores = malloc((numThreads/2)*sizeof(int));
	    for(i=0;i<numThreads;i++){ fscanf(file,"%d",&tmp); }
...
int* na_intra_HT(int numThreads)
{
  ...
  file = fopen("/tmp/output/intra_ht.minas","r");
...
int* na_inter_HT(int numThreads)
{
  ...
  file = fopen("/tmp/output/inter_ht.minas","r");
...
int* na_workStealing_Inter(int numThreads,int helperT)
{
   FILE *file;
  int *cores,tmp,i;

   file = fopen("/tmp/output/inter_ws.minas","r");
...
int* na_workStealing_Intra(int numThreads,int helperT)
{
   FILE *file;
  int *cores,tmp,i;

   file = fopen("/tmp/output/intra_ws.minas","r");
