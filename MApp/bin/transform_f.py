...
#including MAi-array library
	...
	if instruction.find('PROGRAM') != -1 or instruction.find('program'):
		tmpmainfiled.write('use ctof\n')	
	if instruction.find('IMPLICIT NONE') != -1 or instruction.find('implicit none'):
		tmpmainfiled.write('call mai_init(null())\n')
    tmpmainfiled.flush()
    tmpmainfiled.close()	
    mainfiled.close()
    os.rename(tmpmainfile, mainpath)

    searcharray = re.compile(r"'.*' (ARRAY) '.*' .* '.*' '.*'")
...
#find and change variables declaration
	...
	if arraymatch != None: #do only if it is array
		...
		for instruction in currentfiled: #get the declaration line
			var = instruction.find(currentvar)
			if var != -1 and instruction.find(currenttype) != -1:
				if var+len(currentvar) < len(instruction)
					str1 = extract(instruction,currentvar,',')
					if str1 != None:
						newline = instruction.replace(currentvar+str1+',',' ')
				...
				while i <= currentdim:
					if i == (currentdim):
						...
						arraydim+=':,'
				newline = currenttype+',dimension('+arraydim+'), pointer :: +'currentvar+'\n'
				tmpfiled.write(newline)
			else:
			...
			if mainfile == currentfile: #get the declaration line and place MAi functions after that
				if instruction.find(currentvar) != -1 and instruction.find(currenttype) != -1:
					while instruction.find(';') != -1:
						...
						instruction = currentfiled.readline()
					tmpfiled.write(instruction+'\n')
					newline = 'call mai_alloc_'+currentdim+'D_'+currenttype.lower()+'('+currentvar+','
					i = 1
					...
					if policy == 'bind':
						...
						newline = 'call mai_'+policy+'('+currentvar+');'
					tmpfiled.write(newline+'\n')
				else:
					tmpfiled.write(instruction+'\n')
